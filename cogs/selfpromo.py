...
class SelfPromo(commands.Cog):
    ...
    async def selfpromo(self, ctx, message: discord.Message):
        ...
        if database.checkSelfPromoMsg(message.id):
            await ctx.respond("This message has already been reported, thank you!", ephemeral=True)
        else:
            if user:
                ...
                try:
                    ...
                    await ctx.respond("This appears to be an automated message, thanks though! (Please reach out to Chill if you think this was an error)", ephemeral=True)
                    return 0
                ...
                msg = "Woah there,"
            msg = msg + " it looks like you're sharing self-promotion outside of <#" + str(
                SELF_PROMO_CHANNEL) + ">!\n\n"
            ...
            msg = msg + "In the meantime, check out <#" + str(
                ROLE_MENU_CHANNEL) + "> and assign yourself the Artist/Producer tag to unlock some extra channels. Please take a minute to check out our <#" + str(
                RULE_CHANNEL) + ">\n\n"
            ...
            report_msg = "The following message was tagged for self-promotion by <@" + str(
                ctx.author.id) + ">:\n" + message.jump_url + "\n"
